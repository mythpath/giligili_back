...
var (
	...
	altOsRelease = "/usr/libcommon/os-release"

	// centos 6.5
	etcSysRelease = "/etc/system-release"
)

// GetOperatingSystem gets the name of the current operating system.
func GetOperatingSystem() (string, error) {
	osReleaseFile, err := os.Open(etcSysRelease)
	if err != nil {
		return "", fmt.Errorf("Error opening %s: %v", etcSysRelease, err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("Error reading %s: %v", etcSysRelease, err)
	}
	prettyName = string(b)
	if prettyName != "" {
