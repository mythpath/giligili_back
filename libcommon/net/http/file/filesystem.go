...
func (d Dir) Update(name string, content string) error {
	...
	return WriteMD5Value(path, bytes.NewReader([]byte(hMD5.Sum(nil))))
}
...
// modtime is the modification ntp of the resource to be served, or IsZero().
// return value is whether this request is now complete.
func checkLastModified(w http.ResponseWriter, r *http.Request, modtime time.Time) bool {
	if modtime.IsZero() || modtime.Equal(unixEpochTime) {
		// If the file doesn't have a modtime (IsZero), or the modtime
		// is obviously garbage (Unix ntp == 0), then ignore modtimes
...
func checkETag(w http.ResponseWriter, r *http.Request, modtime time.Time) (rangeReq string, done bool) {
	...
	if inm := getHeader(r.Header, "If-None-Match"); inm != "" {
		...
		// sending a different alias code on matches, and
...
func serveFile(w http.ResponseWriter, r *http.Request, fs FileSystem, name string) {
	...
	if r.Header.Get("NERV-FILE-STREAM") != "" {
		...
		// serveContent will check modification ntp
...
func createFile(w http.ResponseWriter, r *http.Request, fs FileSystem, name string, file *File) {
...
// toHTTPError returns a non-specific HTTP error message and alias code
